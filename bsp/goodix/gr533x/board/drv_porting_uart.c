...
static void _uart_0_interrupt_callback(app_uart_evt_t *p_evt) {
    switch(p_evt->type) {
        case APP_UART_EVT_TX_CPLT:
        {
            ...
            rt_sem_release(&(_gr533x_uart0_udata.tx_semaphore));
        }
        break;

        case APP_UART_EVT_RX_DATA:
        {
            _gr533x_uart0_udata.recv_size = p_evt->data.size;
            _gr533x_uart0_udata.recv_pos  = 0;
            _gr533x_uart0_udata.xfer_err  = false;
            rt_sem_release(&(_gr533x_uart0_udata.rx_semaphore));
        }
        break;
        ...
        default:
        {
            _gr533x_uart0_udata.recv_size = 0;
            _gr533x_uart0_udata.recv_pos  = 0;
            _gr533x_uart0_udata.xfer_err  = true;
        }
        break;
    }
}
...
static void _uart_1_interrupt_callback(app_uart_evt_t *p_evt) {
    switch(p_evt->type) {
        case APP_UART_EVT_TX_CPLT:
        {
            ...
            rt_sem_release(&(_gr533x_uart1_udata.tx_semaphore));
        }
        break;

        case APP_UART_EVT_RX_DATA:
        {
            _gr533x_uart1_udata.recv_size = p_evt->data.size;
            _gr533x_uart1_udata.recv_pos  = 0;
            _gr533x_uart1_udata.xfer_err  = false;
            rt_sem_release(&(_gr533x_uart1_udata.rx_semaphore));
        }
        break;

        case APP_UART_EVT_ABORT_TX:
        {
            _gr533x_uart1_udata.xfer_err  = true;
            rt_sem_release(&(_gr533x_uart1_udata.tx_semaphore));
        }
        break;
...
static rt_err_t _configure(struct rt_serial_device *serial, struct serial_configure *cfg) {
    ...
    if(id == APP_UART_ID_0) {
        ...
        _gr533x_uart0_udata.uart_tx_buffer = rt_malloc(cfg->bufsz + 4);
    ...
    if(id == APP_UART_ID_1) {
        ...
        _gr533x_uart1_udata.uart_tx_buffer = rt_malloc(cfg->bufsz + 4);
    ...
    if(_uart_buffer.tx_buf == RT_NULL) {
        rt_kprintf("alloc mem for uart bufsz failed \r\n");
        return RT_ENOMEM;
    }
    ...
    switch(cmd) {
        ...
        case RT_DEVICE_CTRL_SET_INT:
        {
            ...
            if(p_udata->rx_mode != 0) {
                if(p_udata->uart_rx_thread == RT_NULL) {
                    _control_uart_rx_thread(p_udata, THREAD_STARTUP);
                } else {
                    _control_uart_rx_thread(p_udata, THREAD_RESUME);
                }
            } else {
                if(p_udata->uart_rx_thread != RT_NULL) {
                    _control_uart_rx_thread(p_udata, THREAD_SUSPEND);
                }
            }
        ...
        case RT_DEVICE_CTRL_CLR_INT:
        {
            ...
            if(flag & RT_DEVICE_FLAG_INT_RX) {
                if(p_udata->uart_rx_thread != RT_NULL) {
                    _control_uart_rx_thread(p_udata, THREAD_DELETE);
                    p_udata->uart_rx_thread = RT_NULL;
                    p_udata->rx_mode        = 0;
                }
            }
            ret = RT_EOK;
...
static void _uart_rx_thread_entry(void * args) {
    uint16_t ret = 0;
    struct uart_udata_t * p_udata = (struct uart_udata_t *) args;

    rt_kprintf("uart%d rx task ...", p_udata->id);
    ...
    while(1) {
        ...
        if(ret != 0) {
            rt_kprintf("uart%d rx err: %d \r\n", p_udata->id, ret);
            rt_thread_mdelay(1000);
        } else {
            rt_sem_take(&(p_udata->rx_semaphore), RT_WAITING_FOREVER);

            if(p_udata->xfer_err == false) {
...
static bool _control_uart_rx_thread(struct uart_udata_t * p_udata, uint32_t state) {
    ...
    switch(state) {
        case THREAD_STARTUP:
        {
            p_udata->uart_rx_buffer = rt_malloc(p_udata->buffer_size);
            if(p_udata->uart_rx_buffer == RT_NULL) {
                rt_kprintf("alloc uart%d rx buffer fail \r\n", p_udata->id);

                ret = false;
                break;
            }

            p_udata->uart_rx_thread = rt_thread_create( "uart1_rx_task", _uart_rx_thread_entry, (void*)p_udata, 1024, RT_THREAD_PRIORITY_MAX - 10, 8);
            if (p_udata->uart_rx_thread != RT_NULL)
            {
                rt_thread_startup(p_udata->uart_rx_thread);
                ret = true;
            }
        }
        break;
        ...
        {
            if(p_udata->uart_rx_thread != RT_NULL) {
                if(RT_EOK == rt_thread_suspend(p_udata->uart_rx_thread)) {
                    ret = true;
                } else {
                    ret = false;
                }
        ...
        {
            if(p_udata->uart_rx_thread != RT_NULL) {
                if(RT_EOK == rt_thread_resume(p_udata->uart_rx_thread)) {
                    ret = true;
                } else {
                    ret = false;
                }
        ...
        {
            if(p_udata->uart_rx_thread != RT_NULL) {
                if(RT_EOK == rt_thread_delete(p_udata->uart_rx_thread)) {
...
int rt_hw_uart_init(void)
{
    ...
    rt_sem_init(&_gr533x_uart0_udata.rx_semaphore, "uart0_tx", 0, RT_IPC_FLAG_FIFO);
#endif
    ...
    rt_sem_init(&_gr533x_uart1_udata.rx_semaphore, "uart1_rx", 0, RT_IPC_FLAG_FIFO);
#endif
